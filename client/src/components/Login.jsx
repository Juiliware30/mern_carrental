...
const Login = () => {
    ...
    const onSubmitHandler = async (event) =>{
        ...
        try {
             ...
             const {data} = await axios.post(`/api/user/${state}` , state === 'register' ? {name,email,password} : {email,password})
            if(data.success)
            {
                ...
                setToken(data.token)
                console.log('Token set in context:', data.token)
                // Set authorization header immediately for the subsequent fetchUser call
                axios.defaults.headers.common['Authorization'] = `Bearer ${data.token}`
                // Manually fetch user data
                console.log('Manually calling fetchUser')
                const userData = await fetchUser()
                console.log('fetchUser returned:', userData)
                setShowLogin(false)
